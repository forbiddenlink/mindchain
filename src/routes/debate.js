  ...
  asyncHandler(async (req, res) => {
    ...
    if (now - lastGlobalDebateStart < config.debate.startCooldown) {
      throw createError(
        ...
        'rate_limit',
        429,
        { cooldownRemaining: config.debate.startCooldown - (now - lastGlobalDebateStart) }
      );
    }
...
router.get('/:id/messages', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    throw createError(
      'Failed to fetch debate messages',
      'redis',
      503,
      { debateId: id, originalError: error.message }
    );
  }
}));
...
router.post('/start-multiple', 
  ...
  asyncHandler(async (req, res) => {
    ...
    if (topics.length > 5) {
      throw createError('Maximum 5 topics allowed for multi-start', 'validation', 400);
    }

    // Check if we would exceed concurrent limit
    if (activeDebates.size + topics.length > config.debate.maxConcurrentDebates) {
      throw createError(
