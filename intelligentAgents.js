...
class IntelligentAgentSystem {
    ...
    async generateIntelligentResponse(agentId, debateId, topic, conversationHistory = []) {
        ...
        try {
            ...
            const emotionalState = await this.analyzeEmotionalTrajectory(agentId, debateId);
            ...
            const stanceEvolution = await this.calculateStanceEvolution(agentId, debateId, topic);

            // 5. Create enhanced prompt with Redis-powered context
            const enhancedPrompt = this.buildIntelligentPrompt(
                agentId, topic, emotionalState, coalitionStatus, 
                strategicMemory, stanceEvolution, conversationHistory
            );
            ...
            return {
                ...
                metadata: {
                    ...
                    cacheHit,
                    intelligenceLevel: this.calculateIntelligenceLevel(emotionalState, coalitionStatus)
                }
            };
            ...
            return {
                response: "I need to consider this topic more carefully before responding.",
    ...
    async analyzeEmotionalTrajectory(agentId, debateId) {
        try {
            ...
            const recentEmotions = await this.client.ts.range(emotionalKey, '-', '+', { COUNT: 10 }).catch(() => []);
            ...
            return { state: 'analytical', intensity: 0.5, trend: 'stable', timestamp: Date.now() };
        }
    }
    ...
    async evaluateCoalitions(agentId, debateId) {
        try {
            ...
            const existingCoalitions = await this.client.json.get(coalitionKey).catch(() => ({}));
            
            // Analyze potential alliances based on stance similarity
            const agentProfile = await this.client.json.get(`agent:${agentId}:profile`).catch(() => null);
            if (!agentProfile) {
            ...
            await this.client.json.set(coalitionKey, '.', coalitionUpdate).catch(() => {
    ...
    async getStrategicMemory(agentId, debateId) {
        try {
            ...
            const recentMemories = await this.client.xRevRange(memoryKey, '+', '-', { COUNT: 5 }).catch(() => []);
            
            const strategicInsights = recentMemories
                .filter(entry => entry.message.type === 'strategic_insight' || entry.message.type === 'intelligent_response')
                .map(entry => ({
    ...
    async calculateStanceEvolution(agentId, debateId, topic) {
        try {
            ...
            const stanceHistory = await this.client.ts.range(stanceKey, '-', '+', { COUNT: 10 }).catch(() => []);
            ...
            if (stanceHistory.length >= 2) {
                const recent = parseFloat(stanceHistory[stanceHistory.length - 1].value);
    ...
    buildIntelligentPrompt(agentId, topic, emotionalState, coalitionStatus, strategicMemory, stanceEvolution, conversationHistory) {
        const agentProfiles = {
            'senatorbot': 'experienced politician focused on pragmatic policy solutions',
            'reformerbot': 'progressive advocate pushing for transformative change'
        };

        const profile = agentProfiles[agentId] || 'thoughtful participant in political discourse';
...
${conversationHistory.slice(-3).map(msg => `${msg.agentId}: ${msg.message}`).join('\n')}
...
Generate a thoughtful, contextually-aware response that reflects your current emotional state, coalition opportunities, and strategic position. Keep it under 250 words and stay true to your character while demonstrating the intelligence provided by Redis data.`;
    ...
    async updateAgentIntelligence(agentId, debateId, metrics) {
        try {
            ...
            const update = {
                ...
                intelligence_score: this.calculateIntelligenceLevel(metrics.emotionalState, metrics.coalitionStatus)
            };
    ...
    analyzeStrategicPatterns(insights) {
        ...
        if (insights.length >= 2) {
            ...
            if (insights.some(i => i.content.includes('coalition') || i.content.includes('alliance'))) {
...
export async function generateIntelligentMessage(agentId, debateId, topic, conversationHistory = []) {
    return await intelligentAgentSystem.generateIntelligentResponse(agentId, debateId, topic, conversationHistory);
}
